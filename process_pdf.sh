...
# Try to extract the document doi string if it exists
doi="$(./extract_doi.sh ${File})"

# If no doi was inferred exit
if [ -z "${doi}" ]; then
    echo "WARNING: No doi was inferred for the file. Skipping ${File}"
    exit 1
fi

# Generate a new file name for the file complying with the form:
# LastName_Year_Publication.Extension
New_Filename=$(./lookup_crossref.sh "${doi}" |             \
               ./gen_filename.py)                       && \
